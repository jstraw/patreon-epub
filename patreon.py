...
def main(args):
    ...
    chapter_blacklist = story_list['blacklist']


    newbook = book(args.story, 'QuietValerie', 'en')
    ...
    try:
        ...
        age = driver.find_element_by_css_selector(
            "button[data-tag='age-confirmation-button']")
        ...
        author_script = driver.page_source.split('\n')
        author_line = [x for x in author_script if 'creator_id' in x]
        author_id = author_line[0].split(':')[1].strip()
